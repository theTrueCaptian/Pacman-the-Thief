...
public class AStarPathFinder implements PathFinder {
	...
	public AStarPathFinder(Map map, int maxSearchDistance,
		...
		for (int x=0;x<map.getWidthInTiles();x++) {
			for (int y=0;y<map.getHeightInTiles();y++) {
                ...
                pathMap = new int[map.getWidthInTiles()][map.getHeightInTiles()];
                for (int x=0;x<map.getWidthInTiles();x++) {
			for (int y=0;y<map.getHeightInTiles();y++) {
                            //zero indicating no path; 1 indicating a path
				pathMap[x][y] = 0;
			}
		}
	}
            ...
            while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
                ...
                for (int x=-1;x<2;x++) {
                    for (int y=-1;y<2;y++) {
                        ...
                        int xp = x + current.x;
                        int yp = y + current.y;
                        //System.out.println("x"+x+" y"+y+", current.x"+current.x+" current.y"+current.y);
                        if (isValidLocation(sx,sy,xp,yp)) {
                            ...
                            /*if(tx!=guard.sprite.getX() || ty!=guard.sprite.getY()){
                                return null;
                            }*/
                            //guard.move(0.003f, 0.003f);
                        }
	...
	protected boolean isValidLocation( int sx, int sy, int x, int y) {
            ...
            return !invalid;/*
            if(((x < 0) || (y < 0) || (x >= map.getWidthInTiles()) || (y >= map.getHeightInTiles())) || ((sx == x) || (sy == y)) || map.blocked(x,y))
                return false;
            else
                return true;
                */
